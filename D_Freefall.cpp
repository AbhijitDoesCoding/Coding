...
void solve(){
    ...
    auto f = [&](long double x){
        return (long double)b*x + (long double)a/sqrtl(x+1.0L);
    };

    long double real_x = powl((long double)a/(2.0L*b), 2.0L/3.0L) - 1.0L;

    ll center = max(0LL, (ll)real_x);

    long double ans = f(0);

    for(ll i = max(0LL, center-5); i <= center+5; i++){
        ans = min(ans, f(i));
    }
